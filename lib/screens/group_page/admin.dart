...
class Admin extends StatefulWidget {
  ...
  const Admin({Key? key, required this.userGrpDataModel}) : super(key: key);
...
class _AdminState extends State<Admin> {
  ...
  void initState() {
    ...
    if(widget.userGrpDataModel != null) {
      if(widget.userGrpDataModel?.adminGrpId != null && widget.userGrpDataModel?.adminGrpId != "") {
        setState(() {
          adminGroupExists = true;
        });
        context.read<GroupBloc>().add(GetGroupDetailsEvent(grpId: widget.userGrpDataModel!.adminGrpId!));
      }
    }
    context.read<UserGrpDataBloc>().add(GetPreviousGroupsListEvent(uid: currentUser.uid));
    // context.read<UserGrpDataBloc>().add(GetPreviousGroupsListEvent(uid: 'abc'));
      ...
      return SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              // widget.userGrpDataModel?.adminGrpId != null && widget.userGrpDataModel?.adminGrpId != "" ?
              Container(
                color: Colors.white,
                            ...
                            SizedBox(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (BuildContext context, int index) {
                                  ...
                                  return ShowActive(
                                    userId: grpModel.participants[index],);
                                },
                      ...
                      child: BlocBuilder<UserGrpDataBloc, UserGrpDataState>(
                        builder: (context, state) {
                          if (state is PreviousGroupsLoadedState) {
                            List<GroupModel> previousGroups = state.previousGroups;
